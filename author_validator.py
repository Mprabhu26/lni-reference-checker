...
def validate_author_consistency(
    ...
    # Calculate overlap against the CITED (entry) authors. The reference may
    # legitimately abbreviate the list with "et al.", so the denominator is the
    # surnames actually cited - not the union with the (longer) full API list.
    overlap = entry_surnames & api_surnames

    if not entry_surnames:
        return 0.3, "No cited surnames found"

    overlap_ratio = len(overlap) / len(entry_surnames)

    if overlap_ratio >= 0.95:
        return 0.95, f"Authors match well ({len(overlap)}/{len(entry_surnames)} cited surnames)"
    elif overlap_ratio >= 0.80:
        return 0.85, f"Most cited authors match ({len(overlap)}/{len(entry_surnames)} surnames)"
    elif overlap_ratio >= 0.60:
        return 0.70, f"Some cited authors match ({len(overlap)}/{len(entry_surnames)} surnames)"
    elif overlap_ratio >= 0.40:
        return 0.50, f"Few cited authors match ({len(overlap)}/{len(entry_surnames)} surnames)"
    else:
        return 0.30, f"Cited authors significantly different ({len(overlap)}/{len(entry_surnames)} surnames)"
